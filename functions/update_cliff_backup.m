...
%   --------------------------------------------------------------------
    ...
    for i=1:n
        ...
        CLIFF.ds(i)=hypot(CLIFF.x(ip1)-CLIFF.x(im1),CLIFF.y(ip1)-CLIFF.y(im1));
    ...
    for i=1:n
        ...
        CLIFF.dx(i)=-dn*(CLIFF.y(ip1)-CLIFF.y(im1))/CLIFF.ds(i);
        CLIFF.dy(i)= dn*(CLIFF.x(ip1)-CLIFF.x(im1))/CLIFF.ds(i);  %change me!!!
        CLIFF.retreat=CLIFF.retreat+dn; % to use as dx in Trenhaile and Energy_Flux models
    end

    %% Update cliffline positions, all points
    for i=1:n
        CLIFF.x(i)=x0(i)+CLIFF.dx(i);
        CLIFF.y(i)=y0(i)+CLIFF.dy(i);
    end
    if CLIFF.cyclic
        CLIFF.x(n)=CLIFF.x(1);
        CLIFF.y(n)=CLIFF.y(1);
