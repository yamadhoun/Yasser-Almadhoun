    ...
    for i_mc=1:n_mc-1
        for j_mc=i_mc+1:n_mc
            ...
            if i_mc~=j_mc
                ...
                [ xj,yj,n_mc,~,~ ] = get_one_polygon( COAST.x_mc,COAST.y_mc,j_mc );
                
                if length(xi)>2&&length(xj)>2 && ...
                   (abs(sum(xi)-sum(xj))~=0 && abs(sum(yi)-sum(yj))~=0)  % check also if they are not the same!
                    if hypot(xi(end)-xi(1),yi(end)-yi(1))>2*ds0
                        cwi=1;
                    else
                        cwi=get_clockpoly(xi,yi);
                    end
                    
                    if hypot(xj(end)-xj(1),yj(end)-yj(1))>2*ds0
                        cwj=1;
                    else
                    ...
                    %  including the fraction of length of the vertex (umin and vmin, for i and j)
                        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
                        %[xcr,ycr,indi,indj,umin,vmin]=find_crossing(xi,yi,xj,yj);
                        [xcr,ycr,indi,indj]=intersectfn(xi,yi,xj,yj);		
                        integ = fix(indi);
                        frac = mod(abs(indi),1);
