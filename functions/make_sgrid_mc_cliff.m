function [CLIFF]=make_sgrid_mc_cliff(CLIFF,i_mc)
...
%   --------------------------------------------------------------------
    ...
    [ x,y,~,i1,i2 ] = get_one_polygon( CLIFF.x_mc,CLIFF.y_mc,i_mc );
    ar=0;
    s=zeros(size(x));
    s(1)=0;
    for i=2:length(x)
    ...
    while i<=length(snew)
        ds=snew(i)-snew(i-1);
        if ds<CLIFF.ds0/sqrt2    % sqrt(2) instead of 2, so grid sizes stay closer to ds0
            %throw out point i
            if i<length(snew)
        ...
        elseif ds>CLIFF.ds0*sqrt2   % sqrt(2) instead of 2, so grid sizes stay closer to ds0
            %insert point i
    ...
    snew(2:end-1)=CLIFF.smoothfac*snew(1:end-2)+(1.-2*CLIFF.smoothfac)*snew(2:end-1)+CLIFF.smoothfac*snew(3:end);
    if length(snew)>1
        x=interp1(s,x,snew);
        y=interp1(s,y,snew);
    ...
    cyclic=hypot(x(end)-x(1),y(end)-y(1))<CLIFF.ds0;
    ...
    CLIFF.y=y;
    if length(CLIFF.x)<3
       disp(['This is too short a cliff-line segment, #',num2str(i_mc)])  
    end
    CLIFF.n=length(x);
    CLIFF.s=s;
    CLIFF.xq=xq;
    ...
    CLIFF.cyclic=cyclic;
    %CLIFF.ar=ar;
    CLIFF.clockwise=clockwise;
    
    %% insert x,y back into CLIFF.x_mc,CLIFF.y_mc
    if length(snew)>1
        CLIFF.x_mc=[CLIFF.x_mc(1:i1-1),x,CLIFF.x_mc(i2+1:end)];
        CLIFF.y_mc=[CLIFF.y_mc(1:i1-1),y,CLIFF.y_mc(i2+1:end)];
    end
    CLIFF.n_mc=length(find(isnan(CLIFF.x_mc)))+1;
    CLIFF.retreat=zeros(size(CLIFF.x_mc));
end
