...
%   --------------------------------------------------------------------
    ...
    if CHANNEL.used~=0 && ~isempty(CHANNEL.xr_mc)
        ...
        [CHANNEL.xr_mc,CHANNEL.yr_mc]=initialize_grid(CHANNEL.xr_mc,CHANNEL.yr_mc,COAST.ds0);
        ...
        %         xright=COAST.x_mc(d<0);yright=COAST.y_mc(d<0);
            ...
            nmouthpts=4;
            dnmouth=CHANNEL.disch_rate/nmouthpts/COAST.ds0/COAST.h0*TIME.dt;
            ...
            for ir=1:nr
                 if width(ir)<CHANNEL.width(iriv)/2
                    ...
                    dn(ic)=dn(ic)+dw;
                    if samedir1(ir)
                        dn(icp1)=dn(icp1)-dw/2+dnmouth;
                        dn(icp2)=dn(icp2)-dw/2+dnmouth;
                    else
                        dn(icm1)=dn(icm1)-dw/2+dnmouth;
                        dn(icm2)=dn(icm2)-dw/2+dnmouth;
                    end
                    ic=ind2(ir);
                    [icm1,icm2,icp1,icp2]=find_neighbours(COAST.x_mc,COAST.y_mc,ic);
                    dn(ic)=dn(ic)+dw;
