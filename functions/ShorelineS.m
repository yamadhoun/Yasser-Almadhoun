...
%         [CLIFF]=cliffline_change(CLIFF,GROYNE,TIME);                     % <- new function by YA2022DR
        % [CLIFF]=update_cliff(CLIFF,GROYNE,TIME,COAST);                           % <- new function by YA2022DR
    ...
    % [O,TIME.itout]=save_shorelines(O,S,TIME,COAST,WAVE,TRANSP,STRUC,NOUR,GROYNE);

    %% Store data in OUTPUT.MAT (information before cliffline update)
    % [O,TIME.itout]=save_clifflines(O,S,TIME,COAST,CLIFF);
