...
[CLIFF]   = prepare_cliff(S,COAST);                                        % <- new function by 2023
...
while  TIME.tnow<TIME.tend 
    ...
    for i_mc=1:COAST.n_mc 
        ...
        [CLIFF]=make_sgrid_mc_cliff(CLIFF,i_mc);                           % <- new function by 2023
        ...
        %% cliff retreat rate
        [CLIFF]=cliff_retreat_rate(S,COAST,CLIFF,WAVE,TIME);               % <- new function by 2023 
    ...
    for i_mc=1:COAST.n_mc1
        ...
        [CLIFF]=update_cliff(CLIFF,GROYNE,TIME);                           % <- new function by 2023

        %% Coastline change  ++cliff retrat volume drift contribution
        [COAST,GROYNE]=coastline_change(S,COAST,TRANSP,STRUC,GROYNE,TIME,NOUR,CC,CLIFF); % updated for cliff 2023
