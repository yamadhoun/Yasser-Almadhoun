...
%   --------------------------------------------------------------------
    ...
    if isfield(GROYNE,'idcoast')
        for ig=1:size(GROYNE.idcoast,1)
            ...
            if i2start-1==i1end+1
                igroyne = i1end+1;
                COAST.x_mc=[COAST.x_mc(1:i1end),0.5*(COAST.x_mc(i1end)+COAST.x_mc(i2start)),COAST.x_mc(i2start:end)]; 
                COAST.y_mc=[COAST.y_mc(1:i1end),0.5*(COAST.y_mc(i1end)+COAST.y_mc(i2start)),COAST.y_mc(i2start:end)]; 
            else
