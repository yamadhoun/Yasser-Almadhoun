...
ld=0;                                                                      % Width of the land fill behind the shoreline [m]
...
%     [vi,xp_mc,yp_mc,S,xmax,ymax,xmin,ymin,nmax,tsl,tplot,vii]=PlotX(S,x_hard,y_hard,x_mc0,y_mc0,it,x_mc,y_mc,ld,tnow,phiw,tplot,tsl,plot_time,xmax,ymax,xmin,ymin,nmax,xp_mc,yp_mc,vi,vii,xp,yp,QS,xS);
    ...
    S.dttnow = tnow+ S.dt*365; %dt [year]   %calculate the current time after each time step
    S.times(it+1)=tnow;
    S.record_Hs(it+1)=S.Hso;
    S.record_phiw(it+1)=phiw;
%     [CLIFF]=prepare_cliffs(S,S.Hso,phiw,S.dt,'TWL');
end
