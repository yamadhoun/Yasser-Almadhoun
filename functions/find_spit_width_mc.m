...
%         never happen?
            ...
            % Check for shadowing
            [xx1,yy1]=intersectfn(COAST.x_mc,COAST.y_mc,xl,yl);
            ...
            [xx2,~]=intersectfn(COAST.x_mc,COAST.y_mc,xw,yw);
            shadow(i)=~isempty(xx2); 
            if ~isempty(STRUC.x_hard)
                [xx3,~]=intersectfn(STRUC.x_hard,STRUC.y_hard,xw,yw);
                [xx4,~]=intersectfn(STRUC.x_hard,STRUC.y_hard,xl,yl);
...
%         else  % if nan move to the next section , and give the spit logic zero
%             SPIT.spit(i)=false;
%             ii=ii+1; % move to the next section
%             if ii<=COAST.n_mc % apply ifcyclic/twopoint concept
%                 %cyclic_ii=hypot(COAST.x_mc(i2(ii))-COAST.x_mc(i1(ii)),COAST.y_mc(i1(ii))-COAST.y_mc(i2(ii)))<ds;
%                 if COAST.cyclic
%                     COAST.x_mc(i2(ii))=COAST.x_mc(i1(ii));COAST.y_mc(i2(ii))=COAST.y_mc(i1(ii));
%                     dxt(i2(ii))=dxt(i1(ii));dyt(i2(ii))=dyt(i1(ii));
%                 end
%             end
%             
%             if ii>COAST.n_mc
