...
%   --------------------------------------------------------------------
    ...
    if isfield(S,'WVCfile') && ~isempty(S.WVCfile)
        ...
        elseif method=='alongshore_mapping'
            if isfield(var1i,'Kc') && ~isempty(var1i.Kc)
                CLIFF.Kc_grid = var1i.Kc * ones(size(CLIFF.HSo));
            % elseif isfield(var1i,'Kz') && ~isempty(var1i.Kz) && CLIFF.form==1  % form=1 refers to Zhang model
            %     CLIFF.Kz_grid = var1i.Kz * ones(size(WAVE.HSo));
            end
            if isfield(var1i,'d') && ~isempty(var1i.d)
                CLIFF.d_grid = var1i.d * ones(size(CLIFF.HSo));
            end
        end
    ...
    elseif isfield(S,'Hso') && ~isempty(S.Hso) ...
        ...
        CLIFF.Kc_grid=CLIFF.Kconst * ones(size(CLIFF.HSo));
        % CLIFF.Kz_grid=CLIFF.Kz_const * ones(size(CLIFF.HSo));
    end

    % end of section

    %% INTERPOLAT WAVES IN SPACE FROM COASTLINE NEARSHORE TO CLIFF WALL POSITION
    % for model test only: HSo & TP_________________________________________MOVED UP
    % % if isfield(WAVE,'HSo') && ~isempty(WAVE.HSo) ...
    % %         && isfield(WAVE,'TP') && ~isempty(WAVE.TP)
    % %     % WAVE.HSo(n)=WAVE.HSo(nq);
    % %     % WAVE.TP(n)=WAVE.TP(nq);
    % %     var3.HSo=WAVE.HSo;
    % %     var3.TP=WAVE.TP;
    % %     var4.useless=zeros(size(WAVE.TP));
    % %     method='weighted_distance';
    ...
    % %     CLIFF.HSo=var3i.HSo;
    % %     CLIFF.TP=var3i.TP;
    % %     % for model test only: Kc & Kz
    % %     CLIFF.Kc_grid=CLIFF.Kc_const * ones(size(CLIFF.HSo));
    % %     CLIFF.Kz_grid=CLIFF.Kz_const * ones(size(CLIFF.HSo));
    % % end
    ...
    if ~isfield(COAST,'BeachWidth') || isempty(COAST.BeachWidth) 
        opt=1;
        COAST.BeachWidth = CLIFF.BaseBeach;  %CLIFF.BaseBeach;
        CLIFF.Beta = abs(CLIFF.Ej) ./ (COAST.BeachWidth(1,1));
        % CLIFF.Beta = abs(CLIFF.Ej + S.d) ./ (COAST.BeachWidth(1,1) + S.SurfZoneWidth);
        CLIFF.Beta = CLIFF.Beta * ones(size(CLIFF.HSo));
    else
        opt=2;
        CLIFF.Beta=[];
        CLIFF.Beta=COAST.ForeshoreSlope';
        % CLIFF.Beta=CLIFF.Beta(1:length(CLIFF.HSo));
    end
    ...
    switch CLIFF.form
        case 1  % 'Zhang'
            C = Lw ./ CLIFF.TP;
            k = (2*pi)./Lw;
            Cg = C./2 .*(1+(2 .*k .*S.dnearshore) ./sinh(2.*k .*S.dnearshore));  %CLIFF.d
...
%     CLIFF.TWL        = CLIFF.TWL                 ;
%     CLIFF.Om         = CLIFF.Om * TIME.dt        ;
%     CLIFF.dndt_cliff = CLIFF.dndt_cliff * TIME.dt;

%     CLIFF.dndt(end+1) = CLIFF.dndt(end);

    CLIFF.Retreat_Vol_total = CLIFF.dndt * CLIFF.Hc * 1 ; % 1m or gridsize;                          % Retreat total volume rate m3 / m / yr
...
%     CLIFF.Drift_Vol_shore(end+1)=CLIFF.Drift_Vol_shore(end);
